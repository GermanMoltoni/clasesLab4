...
class UsuarioApi extends Usuario{
    ...
    public static function AltaApi($request, $response, $args) {
        ...
        $user = new Usuario($user_data['mail'],$user_data['nombre'],$user_data['apellido'],$user_data['password'],$user_data['usuario'],$path);
        if($user->Alta())
    ...
    public static function ModificarApi($request, $response, $args){
        $user_data = $request->getAttribute('user');
        $path = $request->getAttribute('path');        
        $user = new Usuario($user_data['mail'],$user_data['nombre'],$user_data['apellido'],$user_data['password'],$user_data['usuario'],$path,$user_data['id']);
        if($user->Modificar())
    ...
    public static function BajaApi($request, $response, $args){
        ...
        if(($user = parent::GetUsuarioPorId($id)) != false && $user->id ==$id)
